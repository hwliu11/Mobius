...
# Macro
include (${CMAKE_SOURCE_DIR}/cmake/mobius_macros.cmake)

#-------------------------------------------------------------------------------
# Name:    MOBIUS_RETRIEVE_VERSION
# Purpose: retrieves app version from version file
#-------------------------------------------------------------------------------
function (MOBIUS_RETRIEVE_VERSION VERSION_FILE MAJOR MINOR PATCH)
  file(STRINGS ${VERSION_FILE} VERSION_FILE_DATA)
  foreach (line ${VERSION_FILE_DATA})
    string(REGEX MATCH "MOBIUS_VERSION_MAJOR *[0-9]" match ${line})
    ...
    string(REGEX MATCH "MOBIUS_VERSION_MINOR *[0-9]" match ${line})
    if (NOT "${match}" STREQUAL "")
      separate_arguments(match)
      list(GET match 1 num)
    ...
    string(REGEX MATCH "MOBIUS_VERSION_PATCH *[0-9]" match ${line})
    if (NOT "${match}" STREQUAL "")
      separate_arguments(match)
      list(GET match 1 num)
...
#-------------------------------------------------------------------------------
# Name:    MOBIUS_SUBDIRECTORY_NAMES
# Purpose: retrieves subdirectory names
#-------------------------------------------------------------------------------
function (MOBIUS_SUBDIRECTORY_NAMES MAIN_DIRECTORY RESULT)
...
#-------------------------------------------------------------------------------
# Name:    MOBIUS_FIND_PRODUCT_DIR
# Purpose: searches for a directory containing the 3-rd party product of the
#          given name
#-------------------------------------------------------------------------------
function (MOBIUS_FIND_PRODUCT_DIR ROOT_DIR PRODUCT_NAME RESULT)
  MOBIUS_MAKE_COMPILER_SHORT_NAME()
  MOBIUS_MAKE_COMPILER_BITNESS()
  ...
  MOBIUS_SUBDIRECTORY_NAMES ("${ROOT_DIR}" SUBDIR_NAME_LIST)
