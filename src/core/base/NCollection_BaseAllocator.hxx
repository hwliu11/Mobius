...
*              Note that this object is managed by Handle.
*/
class NCollection_BaseAllocator : public core_OBJECT
{
 public:
  // ---------- PUBLIC METHODS ------------
  mobiusCore_EXPORT virtual void* Allocate (const size_t size);
  mobiusCore_EXPORT virtual void  Free     (void * anAddress);
  ...
  mobiusCore_EXPORT static const t_ptr<NCollection_BaseAllocator>&
    CommonBaseAllocator(void);

  //! Callback function to register alloc/free calls
  mobiusCore_EXPORT static void StandardCallBack
                    (const bool theIsAlloc,
                     const void* theStorage,
                     const size_t theRoundSize,
                     const size_t theSize);

  //! Prints memory usage statistics cumulated by StandardCallBack
  mobiusCore_EXPORT static void PrintMemUsageStatistics();
