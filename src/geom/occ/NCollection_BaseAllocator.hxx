...
class NCollection_BaseAllocator : public Standard_Transient
{
 public:
  // ---------- PUBLIC METHODS ------------
  mobiusGeom_EXPORT virtual void* Allocate (const size_t size);
  mobiusGeom_EXPORT virtual void  Free     (void * anAddress);
  ...
  mobiusGeom_EXPORT static const Handle(NCollection_BaseAllocator)&
    CommonBaseAllocator(void);

  //! Callback function to register alloc/free calls
  mobiusGeom_EXPORT static void StandardCallBack
  ...
  mobiusGeom_EXPORT static void PrintMemUsageStatistics();
