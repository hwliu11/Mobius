...
class NCollection_IncAllocator : public NCollection_BaseAllocator
{
 public:
  ...
  mobiusGeom_EXPORT NCollection_IncAllocator (size_t theBlockSize = DefaultBlockSize);

  //! Setup mutex for thread-safe allocations.
  mobiusGeom_EXPORT void SetThreadSafe (bool theIsThreadSafe = true);

  //! Allocate memory with given size. Returns NULL on failure
  mobiusGeom_EXPORT virtual void* Allocate        (const size_t size) Standard_OVERRIDE;

  //! Free a previously allocated memory. Does nothing
  mobiusGeom_EXPORT virtual void  Free            (void *anAddress) Standard_OVERRIDE;

  //! Diagnostic method, returns the total allocated size
  mobiusGeom_EXPORT size_t        GetMemSize      () const;

  //! Destructor (calls Clean() internally)
  mobiusGeom_EXPORT ~NCollection_IncAllocator     ();

  //! Reallocation: it is always allowed but is only efficient with the
  //! last allocated item
  mobiusGeom_EXPORT void *        Reallocate      (void * anAddress,
  ...
  mobiusGeom_EXPORT void          Reset           (const Standard_Boolean
