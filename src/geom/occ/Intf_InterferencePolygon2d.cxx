   ...
   const Intf_Polygon2d& Obje2)
{
  ...
  double d1 = Obje1.DeflectionOverEstimation(),
...
#define PI2 (myTZones(ltz-decal).GetPoint(tsp))
  ...
  for (ltz=1; ltz<=nbIt; ltz++) {
    ...
    delta2=pr2ma-pr2mi;
    if (delta1<1. && delta2<1.) Only1Seg=true;
    if (delta1==0. || delta2==0.) Only1Seg=true;
...
void Intf_InterferencePolygon2d::Intersect
  (const int iObje1, const int iObje2,
...
// If the length of segment is zero, nothing is done
  double lgT =Sqrt(segT*segT);
  if (lgT<=0.) return;
  double lgO =Sqrt(segO*segO);
  if (lgO<=0.) return;
