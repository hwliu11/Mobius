...
#include <mobius/Standard_Handle.hxx>

#include <mobius/Standard_PErrorHandler.hxx>
#include <mobius/Standard_JmpBuf.hxx>
#include <mobius/Standard_HandlerStatus.hxx>
#include <mobius/Standard_ThreadId.hxx>
...
public:
  ...
  mobiusGeom_EXPORT Standard_ErrorHandler();
  
  //! Unlinks and checks if there is a raised exception.
  mobiusGeom_EXPORT void Destroy();
  ...
  mobiusGeom_EXPORT void Unlink();
  
  //! Returns "True" if the caught exception has the same type
  //! or inherits from "aType"
  mobiusGeom_EXPORT Standard_Boolean Catches (const Handle(Standard_Type)& aType);
  
  //! Returns label for jump
  Standard_JmpBuf& Label() { return myLabel; }
  
  //! Returns the current Error.
  mobiusGeom_EXPORT Handle(Standard_Failure) Error() const;
  
  //! Returns the caught exception.
  mobiusGeom_EXPORT static Handle(Standard_Failure) LastCaughtError();
  
  //! Test if the code is currently running in a try block
  mobiusGeom_EXPORT static Standard_Boolean IsInTryBlock();
...
private:
  ...
  mobiusGeom_EXPORT static void Abort (const Handle(Standard_Failure)& theError);
  
  //! Set the Error which will be transmitted to "calling routines".
  mobiusGeom_EXPORT static void Error (const Handle(Standard_Failure)& aError);
  
  //! Returns the current handler (closest in the stack in the current execution thread)
  mobiusGeom_EXPORT static Standard_PErrorHandler FindHandler (const Standard_HandlerStatus theStatus, const Standard_Boolean theUnlink);
...
public:
  ...
  public:
    ...
    mobiusGeom_EXPORT virtual void DestroyCallback() = 0;
