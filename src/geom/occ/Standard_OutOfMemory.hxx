...
{
  mobiusGeom_EXPORT void Throw () const Standard_OVERRIDE;

public:

  //! Constructor is kept public for backward compatibility
  mobiusGeom_EXPORT Standard_OutOfMemory(const Standard_CString theMessage = 0);

  //! Returns error message
  mobiusGeom_EXPORT Standard_CString GetMessageString() const Standard_OVERRIDE;
  
  //! Sets error message
  mobiusGeom_EXPORT void SetMessageString (const Standard_CString aMessage) Standard_OVERRIDE;

  //! Raises exception with specified message string
  mobiusGeom_EXPORT static void Raise(const Standard_CString theMessage = "");

  //! Raises exception with specified message string
  mobiusGeom_EXPORT static void Raise(Standard_SStream& theMessage);

  //! Returns global instance of exception
  mobiusGeom_EXPORT static Handle(Standard_OutOfMemory) NewInstance (Standard_CString theMessage = "");
