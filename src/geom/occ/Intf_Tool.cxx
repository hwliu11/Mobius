			  ...
			  Bnd_Box2d& boxHypr2d)
{
  ...
  if (nbPi>0) {
	...
	if (sinan>0.) {
	  ...
	  else ipmin =  (int)(beginOnCurve[nbSeg-1]);

	  int ipmax;
	  if(endOnCurve[nbSeg-1] > 10.) ipmax = 10;
	  else ipmax =  (int)(endOnCurve[nbSeg-1]);
...
int Intf_Tool::Inters2d(const gp_Hypr2d& theCurv,
				    const Bnd_Box2d& Domain)
{
  int nbpi=0;
  int npi;
...
int Intf_Tool::Inters2d(const gp_Parab2d& theCurv,
				    const Bnd_Box2d& Domain)
{
  int nbpi=0;
  int npi;
...
int Intf_Tool::Inters3d(const gp_Hypr& theCurv,
				    const Bnd_Box& Domain)
{
  int nbpi=0;
  int npi;
