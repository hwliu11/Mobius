...
public: \
  typedef Base base_type; \
  static const char* get_type_name () { return #Class; OCCT_CHECK_BASE_CLASS(Class,Base) } \
  mobiusGeom_EXPORT static const Handle(Standard_Type)& get_type_descriptor (); \
  mobiusGeom_EXPORT virtual const Handle(Standard_Type)& DynamicType() const Standard_OVERRIDE;
...
public:
  ...
  mobiusGeom_EXPORT bool SubType (const Handle(Standard_Type)& theOther) const;

  //! Returns True if this type is the same as theOther, or inherits from theOther.
  //! Note that multiple inheritance is not supported.
  mobiusGeom_EXPORT bool SubType (const Standard_CString theOther) const;

  //! Prints type (address of descriptor + name) to a stream
  mobiusGeom_EXPORT void Print (std::ostream& theStream) const;
  ...
  mobiusGeom_EXPORT ~Standard_Type ();
...
inline std::ostream& operator << (std::ostream& theStream, const Handle(Standard_Type)& theType) 
{
